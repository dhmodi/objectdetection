...
@application.route('/htmlimage', methods=['POST'])
def upload_file():
...
@application.route('/similarimage', methods=['POST'])
def similarity_image():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename) and file2 and allowed_file(file2.filename):
            ...
            file.save(os.path.join(application.config['UPLOAD_FOLDER'], filename))
            file2.save(os.path.join(application.config['UPLOAD_FOLDER'], filename2))
            # return redirect(url_for('upload_file',
            #                         filename=filename))
            img_path = UPLOAD_FOLDER + "/" + filename
...
if __name__ == '__main__':
    ...
    # static = Environment(application)
    #
